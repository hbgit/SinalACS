...
/// fundo do `Scaffold` (`AcsColors.background`), quando texto de risco e de
/// status é renderizado dentro de `Card` (`AcsColors.surfaceRaised`) ou de
/// `AppBar` (`AcsColors.surface`) — por isso cada linha aqui declara a
/// SUPERFÍCIE real, não só o par de cores.
void main() {
  ...
  const cases = <(String, Color, Color, double)>[
    ('branco sobre scaffold', Colors.white, AcsColors.background, normalText),
    ('branco sobre card', Colors.white, AcsColors.surfaceRaised, normalText),
    ('yellow sobre card (badge de risco amarelo)', AcsColors.yellow, AcsColors.surfaceRaised, normalText),
    ('green sobre card (badge de risco verde)', AcsColors.green, AcsColors.surfaceRaised, normalText),
    ...
    ('redOnSurface sobre card', AcsColors.redOnSurface, AcsColors.surfaceRaised, normalText),
    ('accentOnSurface sobre card', AcsColors.accentOnSurface, AcsColors.surfaceRaised, normalText),
    ('redOnSurface sobre scaffold', AcsColors.redOnSurface, AcsColors.background, normalText),
    ('accentOnSurface sobre scaffold', AcsColors.accentOnSurface, AcsColors.background, normalText),
    ('accentOnSurface sobre appbar', AcsColors.accentOnSurface, AcsColors.surface, normalText),
  ...
  test('red (fill) usado como texto sobre card fica abaixo de 4.5:1', () {
    expect(contrastOn(AcsColors.red, AcsColors.surfaceRaised), lessThan(normalText));
  });

  test('accent (fill) usado como texto sobre card fica abaixo de 4.5:1', () {
    expect(contrastOn(AcsColors.accent, AcsColors.surfaceRaised), lessThan(normalText));
  });
  ...
  test('#EF4444 (correção sugerida pelo relatório antigo) continua abaixo de 4.5:1 sobre card', () {
    ...
    final ratio = contrastOn(const Color(0xFFEF4444), AcsColors.surfaceRaised);
