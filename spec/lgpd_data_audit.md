...
| | `codeHash` | `text` | Crítico — credencial | HMAC-SHA-256 do código, campo de domínio próprio | O código em claro nunca é persistido; existe só entre a geração e o envio. Usa o mesmo `CPF_HASH_PEPPER` de `users.cpfHash`, com separação de domínio (`sinalacs:otp:v1:`) — o parecer do §2.1 sobre SHA-256 sem salt **não** se aplica a este campo, pelo mesmo motivo que não se aplica ao CPF migrado: há segredo de servidor na chave. |
...
| **enrollment_tokens** | `id` | `uuid` | Pseudonimizado | UUID da linha | — |
| | `tokenHash` | `text` | Crítico — credencial | SHA-256 do token, **sem pepper e sem domínio** (`OnboardingService._hash`) | O token em claro existe só entre a geração (devolvido ao ACS para virar QR Code) e a leitura pelo paciente, e nunca é persistido. A disciplina de persistência é a mesma de `users.cpfHash` — só o hash vai ao banco —, mas o **mecanismo não é**: ali é HMAC com `CPF_HASH_PEPPER` e domínio, aqui é SHA-256 nu. O parecer do §2.1 **não** se aplica, por um motivo diferente do `codeHash`: o token são 32 bytes de `Random.secure()`, então não há espaço de busca a enumerar (o CPF tem $10^9$). Registrado em 2026-09-19 para o inventário ficar exaustivo de fato — era a única tabela de domínio fora dele. |
| | `patientId` | `uuid` | Pseudonimizado | Chave estrangeira (`patients.id`) | Liga o convite ao paciente que ele ativa; o onboarding não cria identidade nova. |
| | `microAreaId` | `uuid` | Pseudonimizado / Territorial | Microárea do ACS que gerou o convite | É o território que a sessão emitida por `completeEnrollment` carrega — INV-01. |
| | `createdByAcsId` | `uuid` | Pseudonimizado | Chave estrangeira (`acs.id`) | Quem gerou o convite. O fluxo de onboarding **não** escreve em `audit_logs` (não chama `AuditTrail`): nem a emissão nem a conclusão do convite entram na cadeia — o que ele grava é `consent_logs`, assinado com o mesmo `AUDIT_CHAIN_SECRET` (`ConsentSignature`). |
| | `createdAt` / `expiresAt` / `consumedAt` | `timestamp without time zone` | Metadado Técnico / de Segurança | Janela de validade (15 min) e consumo atômico | `consumedAt` marca o uso único; o consumo é um `UPDATE` condicional no Postgres, não uma leitura seguida de escrita. |
...
* **`users.birthDate` como Fator de Autenticação (RF01):** Conforme definido no requisito RF01 do PRD, a data de nascimento atua conjuntamente com o CPF como credencial no fluxo de *Login Passwordless*.
  * **Minimização de Tipo:** Como a autenticação exige a data exata, o truncamento para ano/idade é inviável sem quebrar o login. No entanto, o tipo de dado atual no PostgreSQL (`timestamp without time zone`) deve ser alterado para o tipo `date`, descartando horas, minutos e segundos desnecessários. **Não atendido** (2026-09-18): o Serverpod não expõe tipo de coluna `date` — no `.spy.yaml`, `type=` é o tipo *Dart* do campo, e `DateTime` mapeia fixo para `timestamp without time zone` (`ColumnType` não tem a variante, e o gerador recusa o modelo). As alternativas foram pesadas e rejeitadas: um `ALTER ... USING "birthDate"::date` escrito à mão deixaria o schema-de-registro mentindo (e um `create-repair-migration` reverteria a coluna), e guardar a data como `String` perderia a tipagem em toda a cadeia. Consequência prática: a coluna carrega hora — sempre meia-noite UTC, que é o que o app envia — e o serviço compara por dia (ano/mês/dia em UTC), então o login funciona e a recomendação **segue em aberto**.
  * **Risco de Correlação:** Armazenar a data de nascimento em texto claro ao lado de um `cpfHash` frágil amplia exponencialmente o risco de reidentificação por cruzamento com bases públicas vazadas. A proteção de `users.cpfHash` via HMAC-SHA-256 com segredo (*pepper*) do backend torna-se mandatória para impedir que a credencial de login do paciente seja quebrada em caso de vazamento do banco. **Atendido** (2026-09-18, `docs/superpowers/plans/2026-09-18-rf01-login-passwordless-otp.md`): `users.cpfHash` passou a ser HMAC-SHA-256 com `CPF_HASH_PEPPER` (`HmacCpfHasher`), calculado no servidor — o seed de desenvolvimento grava o HMAC de CPFs sintéticos e o login compara HMAC —, e o índice da coluna virou único. O pepper é tratado como os demais segredos: fallback público só em `development`, boot recusado fora dele. A recomendação correspondente, no §2.6, está marcada como atendida. **Nota de seed (2026-09-19):** a conferência de que as linhas de `users` guardam 64 caracteres hexadecimais vale para as linhas dos **pacientes**; a do ACS fica com um literal (`development-acs`), e isso é deliberado — `bin/seed_cpf_hashes.dart` a exclui porque `verifyOtp` sempre emite `role: UserRole.patient`, e um CPF naquela linha faria o login passwordless emitir um token *de paciente* com o id e a microárea do ACS. O literal não é credencial: `findByCpfHash` procura o HMAC de um CPF, e nenhum HMAC é aquela string — é resíduo de dado sintético de desenvolvimento, não um caminho de escrita do produto.
...
| :--- | :--- | :--- |
| `users.cpfHash` | ~~Espaço amostral $10^9$; reversível por força bruta em segundos.~~ **Atendido** desde o RF01. | ~~Migrar para **HMAC-SHA-256** utilizando segredo corporativo (*pepper*) injetado via variável de ambiente restrita ao backend (`CPF_HASH_PEPPER`). O cálculo deve ser feito exclusivamente pelo servidor.~~ Feito: `HmacCpfHasher` calcula o HMAC no servidor com `CPF_HASH_PEPPER`, o índice da coluna virou único e nenhum caminho de escrita grava SHA-256 nu. Ver §2.5. |
| `users.birthDate` | Tipo com precisão excessiva de horário (`timestamp`) em credencial sensível. | Alterar a coluna para o tipo SQL `date`, preservando o valor exato para o login passwordless (RF01) e eliminando horas/minutos/segundos. **Não atendido** (2026-09-18): o Serverpod não expõe tipo de coluna `date` — `DateTime` mapeia fixo para `timestamp without time zone` e o gerador recusa `type=date`. A coluna carrega meia-noite UTC e o serviço compara por dia; a recomendação segue em aberto (ver §2.5). |
