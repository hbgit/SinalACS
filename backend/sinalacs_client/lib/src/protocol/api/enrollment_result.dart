/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters
// ignore_for_file: invalid_use_of_internal_member

// ignore_for_file: no_leading_underscores_for_library_prefixes

import 'package:serverpod_client/serverpod_client.dart' as _i1;

/// Resultado da conclusão do onboarding pelo paciente: já vem com a sessão
/// ativa, mesma forma de `DevelopmentLoginResult` — reaproveita o mecanismo
/// de token existente porque a autenticação institucional (RF01/RF07)
/// segue como decisão separada e bloqueada.
abstract class EnrollmentResult implements _i1.SerializableModel {
  EnrollmentResult._({
    required this.accessToken,
    required this.tokenType,
  });

  factory EnrollmentResult({
    required String accessToken,
    required String tokenType,
  }) = _EnrollmentResultImpl;

  factory EnrollmentResult.fromJson(Map<String, dynamic> jsonSerialization) {
    return EnrollmentResult(
      accessToken: jsonSerialization['accessToken'] as String,
      tokenType: jsonSerialization['tokenType'] as String,
    );
  }

  String accessToken;

  String tokenType;

  /// Returns a shallow copy of this [EnrollmentResult]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  EnrollmentResult copyWith({
    String? accessToken,
    String? tokenType,
  });
  @override
  Map<String, dynamic> toJson() {
    return {
      '__className__': 'EnrollmentResult',
      'accessToken': accessToken,
      'tokenType': tokenType,
    };
  }

  @override
  String toString() {
    return _i1.SerializationManager.encode(this);
  }
}

class _EnrollmentResultImpl extends EnrollmentResult {
  _EnrollmentResultImpl({
    required String accessToken,
    required String tokenType,
  }) : super._(
         accessToken: accessToken,
         tokenType: tokenType,
       );

  /// Returns a shallow copy of this [EnrollmentResult]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  @override
  EnrollmentResult copyWith({
    String? accessToken,
    String? tokenType,
  }) {
    return EnrollmentResult(
      accessToken: accessToken ?? this.accessToken,
      tokenType: tokenType ?? this.tokenType,
    );
  }
}
